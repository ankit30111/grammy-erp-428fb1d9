...
export function PlantLinesDialog({
  ...
  const create = useMutation({
    mutationFn: async (form: FormState) => {
      ...
      const { error } = await supabase.from("production_lines").insert({
        ...
        location_building: form.location_building.trim() || null,
        location_floor: form.location_floor.trim() || null,
        location_bay: form.location_bay.trim() || null,
      });
      if (error) throw error;
    },
  ...
  const update = useMutation({
    ...
    onSuccess: () => { toast.success("Updated"); setEdit(null); invalidate(); },
    onError: (e: any) => toast.error(e?.message ?? "Failed"),
  });
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 pt-2">
          <div className="flex justify-end">
            <Button size="sm" onClick={() => setAddOpen(true)} className="gap-2" disabled={!plantId}>
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {rows.map((r) => {
                const loc = [r.location_building, r.location_floor, r.location_bay]
                  .filter(Boolean).join(" · ");
                return (
                  <div
                    key={r.id}
          ...
          initial={
            edit
              ? {
                  ...
                  notes: edit.notes ?? "",
                  location_building: edit.location_building ?? "",
                  location_floor: edit.location_floor ?? "",
                  location_bay: edit.location_bay ?? "",
                }
              : undefined
          }
        ...
        <div className="space-y-4 pt-2">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-1">
              ...
              />
              <Input
                value={form.location_bay}
                onChange={(e) => set("location_bay", e.target.value)}
                placeholder="Bay"
              />
            </div>
          </div>
